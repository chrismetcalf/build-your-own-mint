...
exports.transformTransactionsToUpdates = function(transactions) {
  ...
  const updates = transactions.map(function(transaction, i) {
    return {
      range: `Transactions!A${i + 2}:D${i + 2}`,
      values: [Object.values(transaction)]
    }
  });
...
exports.transformBalancesToAppends = function(balances) {
  const timestamp = moment().format("MM/DD/YYYY HH:mm:SS");

  appends = balances.map(function(balance, i) {
    return [timestamp].concat(Object.values(balance))
  });
